...
import type { EditorState, TextFormat, RichEditorOptions, EditorRange } from '../types/editor.types.js';
import type { RichEditorInstance } from '../core/Extension.js';

export class RichEditor {
  private container: HTMLElement;
  private toolbarContainer!: HTMLElement;
  private contentContainer!: HTMLElement;
  private htmlPreviewContainer!: HTMLElement;
  private htmlPreviewContent!: HTMLElement;
  private htmlPreviewCode!: HTMLElement;
  private engine: EditorEngine;
  private toolbar: Toolbar | null = null;
  private content: EditorContent | null = null;
  private selectionManager: SelectionManager | null = null;
  private htmlPreviewManager!: HtmlPreviewManager;
  private formatStateManager!: FormatStateManager;
  private pasteHandler!: PasteHandler;
  private commandHandler!: CommandHandler;
  private extensionManager!: ExtensionManager;
  private isComposing: boolean = false;
  private isRendering: boolean = false;
  private htmlPreviewTimer: number | null = null;
   ...
   */
  private createEditorInstance(): RichEditorInstance {
    return {
      getContent: () => this.getContent(),
      setContent: (html: string) => this.setContent(html),
      getState: () => this.getState(),
      focus: () => this.focus(),
      executeCommand: (command: string, value?: string | number) => this.handleCommand(command, value),
  ...
  private setViewMode(mode: 'editor' | 'preview' | 'both'): void {
  ...
  private updateHtmlPreviewNow(): void {
    this.htmlPreviewManager.update();
  }
  ...
  private initializeContent(): void {
    this.content = new EditorContent(
      ...
      (content, isHtml) => this.handlePaste(content, isHtml),
    );

    // Initialize managers that need content
    const contentElement = this.content.getContainer();
    this.formatStateManager = new FormatStateManager((format: TextFormat) => {
      if (this.toolbar) {
  ...
  private initializeSelectionManager(): void {
  ...
  private isBoldShortcut(event: KeyboardEvent): boolean {
    return event.key === 'b';
  }

  private isItalicShortcut(event: KeyboardEvent): boolean {
    return event.key === 'i';
  }

  private isUnderlineShortcut(event: KeyboardEvent): boolean {
    return event.key === 'u';
  }

  private handleCommand(command: string, value?: string | number): void {
    if (!this.content) {
      return;
    }
  ...
  private debouncedUpdateFormatState(): void {
    this.formatStateManager.debouncedUpdate();
  }

  private handleInput(_text: string): void {
  ...
  private debouncedUpdateHtmlPreview(): void {
  ...
  private updateHtmlPreviewOnSelectionChange(): void {
  ...
  private handlePaste(content: string, isHtml: boolean): void {
    if (!this.content) {
      return;
    }
   ...
   * ```
   */
  public registerExtension(extension: import('../core/Extension.js').Extension): void {
    this.extensionManager.register(extension);
